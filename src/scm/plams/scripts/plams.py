...
from scm.plams import init, config, load_all, log, finish, __version__
from scm.plams import *  # noqa: F401, F403
...
def main():
    ...
    parser.add_argument(
        "--version", action="version", version="PLAMS " + str(VERSION), help="show version number and exit"
    )
    parser.add_argument(
        "-p",
        "--path",
    ...
    # execute input script as if it were __main__
    try:
        plams_namespace["__name__"] = "__main__"
        exec(
            compile(
