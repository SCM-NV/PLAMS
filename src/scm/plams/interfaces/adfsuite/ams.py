...
class AMSResults(Results):
    ...
    def get_molecule_count_history(
        ...
        *species* is ``None``, all detected formulas are returned; otherwise, only the requested
        ...
        :param species: molecular formulas to include, or ``None`` for all detected formulas
        :return: one-based frame indices, times in fs, and population arrays keyed by formula
        :raises KeyError: if molecule or MD time history is unavailable
        :raises ValueError: if a requested formula is unknown or the histories have different lengths
        """
    ...
    class EnergyLandscape:
        ...
        class State:
            ...
            def display_id(self) -> int:
                """Return the original identifier when retained, otherwise the current identifier."""
        ...
        def select_states(
            ...
            :param state_ids: one-based identifiers of states to copy
            :param keep_original_ids: display the source identifiers in copied states
            :return: landscape containing copies of the selected states
            :raises ValueError: if a state identifier is outside the landscape
            """
        ...
        def accessible_states(
            ...
            :param start_state_id: one-based identifier of the starting state
            :param energy_window: maximum energy above the starting state allowed along a path
            :param unit: unit of ``energy_window``
            :param keep_original_ids: display the source identifiers in copied states
            :return: landscape containing copies of all accessible states
            :raises ValueError: if the start identifier is invalid or the energy window is negative
            """
...
class AMSJob(SingleJob):
    ...
    def _serialize_input(self, special: Dict[Type, Callable[[Any], str]]) -> str:
        ...
        def serialize(
            ...
            :param parent_key: parent block name
