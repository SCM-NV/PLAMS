...
__all__ = ["QC2Job", "QC2Results", "QC2Settings"]


class QC2Settings(Settings):
...
class QC2Results(Results):
    """A Subclass of the Results class to access the results given in the QC2 output. Can be extended as required."""
...
class QC2Job(SingleJob):
    """A Subclass of the SingleJob class representing a computational job with QC2."""

    _result_type = QC2Results

    def _get_ready(self):
        """If molecule is defined, xyz files are generated accordingly. If there is data to transfer between ADF and QC2, file formats are converted."""
        ...
        ams_job_dirs = [
            d for d in base_path.iterdir() if d.is_dir() and any(f.startswith("QC2AMS") for f in os.listdir(d))
        ]

        if ams_job_dirs:
            self._translate_AMSJob_to_QC2(ams_job_dirs[0])  # Processes the first matching directory
        return

    def _translate_AMSJob_to_QC2(self, AMSJob_folder):
        ...
        n_basis_func_file_source = AMSJob_folder / "QC2AMS.nBasisFunc.txt"
        n_basis_func_file_dest = Path(self.path) / "QC2AMS.nBasisFunc.txt"
        ...
        convert_to_HDF5(
            AMSJob_folder / "QC2AMS.ERIs.bin", "QC2AMS.ERIs.h5", "ERIs", (dim_size, dim_size, dim_size, dim_size)
        )
        convert_to_HDF5(AMSJob_folder / "QC2AMS.orbEnergies.bin", "QC2AMS.orbEnergies.h5", "orbitalEnergies")
