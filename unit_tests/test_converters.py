...
class TestVaspConverter:
    ...
    def test_singlepoint_conversion(self, single_point_vasp):
        ...
        rkf_dir = Path(vasp_output_to_ams(str(single_point_vasp), overwrite=True))
    ...
    def test_NpT_conversion(self, npt_vasp):
        skip_if_no_ams_installation()
        rkf_dir = Path(vasp_output_to_ams(str(npt_vasp), overwrite=True))
        reader = KFReader(rkf_dir / "ams.rkf")
    ...
    def test_relaxation_conversion(self, relaxation_vasp):
        skip_if_no_ams_installation()
        rkf_dir = Path(vasp_output_to_ams(str(relaxation_vasp), overwrite=True))
        reader = KFReader(rkf_dir / "ams.rkf")
