...
class GammaJob(MultiJob):
    ...
    def prerun(self):
        ...
        for charge, spin in zip(charges, self.spins):
            ...
            newjob.settings.input.Engine.XC.RangeSep = "gamma={:f}".format(self.gamma)
            if spin != 0:
                newjob.settings.input.Engine.Unrestricted = True
                newjob.settings.input.Engine.SpinPolarization = spin
            # print("\n==============================================================")
            # print(newjob.name)
            # print(f"{spin=}")
            # print(newjob.get_input())
            # print("==============================================================\n")
...
s = Settings()
s.input = AMS()
s.input.Task = "SinglePoint"
s.input.Engine = ADF()
s.input.Engine.Basis.Type = "DZP"
s.input.Engine.Basis.Core = "None"
s.input.Engine.XC.GGA = "PBE"
s.input.Engine.XC.XCFun = True
s.runscript.nproc = 1
