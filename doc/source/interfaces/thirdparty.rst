...
From the PLAMS perspective each such interface is just a subclass of |SingleJob| that implements a way of producing a working runscript (|get_runscript|) and a valid input file (|get_input|) for a particular binary based on the contents of job settings.
...
The last chapter presents a slightly different kind of interfaces, so-called *molecule interfaces*.
